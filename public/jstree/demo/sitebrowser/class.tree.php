...
class tree
{
    ...
    public function reconstruct($analyze = true)
    {
        ...
        if ($this->options['data_table'] != $this->options['structure_table']) {
            ...
            $this->db->query("
				...
				SELECT " . $this->options['structure']['id'] . " " . str_repeat(", " . $this->options['structure']['id'],
                    count($this->options['data']) - 1) . "
    ...
    public function rm($id)
    {
        ...
        if ($this->options['data_table']) {
            ...
            $sql[] = "DELETE FROM " . $this->options['data_table'] . " WHERE " . $this->options['data2structure'] . " IN (" . implode(',',
                    $tmp) . ")";
        }
    ...
    public function cp($id, $parent, $position = 0)
    {
        ...
        $sql[] = "
			...
			SELECT " . implode(',',
                array_values($fields)) . " FROM " . $this->options['structure_table'] . " WHERE " . $this->options['structure']["id"] . " IN (" . implode(",",
                $tmp) . ")
        ...
        if (count($fields)) {
            try {
                $this->db->query("
						INSERT INTO " . $this->options['data_table'] . " (" . $this->options['data2structure'] . "," . implode(",",
                        $fields) . ")
						SELECT " . $iid . "," . implode(",",
                        $fields) . " FROM " . $this->options['data_table'] . " WHERE " . $this->options['data2structure'] . " = " . $id[$this->options['data2structure']] . "
        ...
        foreach ($new_nodes as $k => $node) {
            ...
            if (count($fields)) {
                ...
                foreach ($fields as $f) {
					...
					INSERT INTO " . $this->options['data_table'] . " (" . $this->options['data2structure'] . "," . implode(",",
                            $fields) . ")
					SELECT " . (int)$node[$this->options['structure']["id"]] . "," . implode(",",
                            $fields) . " FROM " . $this->options['data_table'] . "
