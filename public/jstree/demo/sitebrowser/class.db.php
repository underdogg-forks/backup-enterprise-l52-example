...
namespace vakata\database {
    ...
    class Query
    {
        ...
        public function execute($vars = array())
        {
            ...
            $this->num = (is_object($this->rsl) || is_resource($this->rsl)) && is_callable(array(
                $this->drv,
                'nf'
            )) ? (int)@$this->drv->nf($this->rsl) : 0;
    ...
    class mysql_driver extends ADriver
    {
        ...
        public function connect()
        {
            ...
            if ($this->lnk === false || !mysql_select_db($this->settings->database,
                    $this->lnk) || !mysql_query("SET NAMES '" . $this->settings->charset . "'", $this->lnk)) {
    ...
    class postgre_driver extends ADriver
    {
        ...
        public function execute($sql, $data = array())
        {
            ...
            $temp = (is_array($data) && count($data)) ? pg_query_params($this->lnk, $sql,
                $data) : pg_query_params($this->lnk, $sql, array());
            if (!$temp) {
    ...
    class oracle_driver extends ADriver
    {
        ...
        {
            $this->lnk = ($this->settings->persist) ?
                @oci_pconnect($this->settings->username, $this->settings->password, $this->settings->servername,
                    $this->settings->charset) :
                @oci_connect($this->settings->username, $this->settings->password, $this->settings->servername,
                    $this->settings->charset);
            if ($this->lnk === false) {
